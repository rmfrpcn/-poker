...
public class CardTest2 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (suit.equals("all")) { //all
			for (int i = 0; i <= 51; i++) {
				suitFlag[i] = true;
			}
		} else if (suit.equals("spade")) { //spade
			for (int i = 0; i <= 12; i++) {
				suitFlag[i] = true;
			}
		} else if (suit.equals("heart")) { //heart
			for (int i = 13; i <= 25; i++) {
				suitFlag[i] = true;
			}
		} else if (suit.equals("diamond")) { //diamond
			for (int i = 26; i <= 38; i++) {
				suitFlag[i] = true;
			}
		} else if (suit.equals("clover")) { //clover
			for (int i = 39; i <= 51; i++) {
				suitFlag[i] = true;
			}
		}
		...
		for (int j = 0; j <= 12; j++) { // 0-12が含まれている個所を"true"
			if (numberlist.contains(String.format("%d", j))) { //jをStringに変換
				for (int i = j; i <= 51; i += 13) { // 13個おきにiを増やす
					numberFlag[i] = true;
				}
			}
		}
